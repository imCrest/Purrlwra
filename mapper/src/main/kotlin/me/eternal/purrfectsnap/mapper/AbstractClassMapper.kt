...
abstract class AbstractClassMapper(
    ...
    fun writeFromJson(json: JsonObject): List<String> {
        ...
        values.forEach { (key, value) ->
            // Skip null or empty string values - they're optional fields that weren't found
            ...
            // Only warn if the property was attempted to be serialized but resulted in null
            if (json.has(key) && json.get(key).let { it.isJsonNull || (it.isJsonPrimitive && it.asString == "null") }) {
                warns.add("Failed to serialize property $key in $mapperName")
            }
        }
        return warns
